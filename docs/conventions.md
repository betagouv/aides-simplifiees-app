# Code Conventions - Aides Simplifiées
...
### Backend (Japa)
For backend controllers, services, and models.

```typescript
// tests/unit/example_service.spec.ts
import { test } from '@japa/runner'

test.group('Example Service', () => {
  test('it handles business logic', async ({ assert }) => {
    // ...
  })
})
```
...
### Frontend (Vitest)
For Vue components, composables, and utility functions.
Use `~` alias for `inertia/` directory.

```typescript
// tests/frontend/example_component.spec.ts
