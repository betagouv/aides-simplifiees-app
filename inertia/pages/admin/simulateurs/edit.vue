...
} = usePage<InferPageProps<AdminController, 'editSimulateur'>>()

const { setBreadcrumbs } = useBreadcrumbStore()
setBreadcrumbs([
  { text: 'Administration', to: '/admin' },
  { text: 'Simulateurs', to: '/admin/simulateurs' },
  { text: `Modifier - ${simulateur.title}`, to: `/admin/simulateurs/${simulateur.id}/edit` },
])
...
const form = useForm({
  ...
  shortTitle: simulateur.shortTitle || '',
  pictogramPath: simulateur.pictogramPath || '',
  status: simulateur.status,
})

function handleSubmit() {
  form.post(`/admin/simulateurs/${simulateur.id}`)
}
</script>

<template>
  <Head :title="`Modifier - ${simulateur.title} | Admin`" />
