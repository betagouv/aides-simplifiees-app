...
Publicodes packages are organized in the `packages/` folder:

```
packages/
├── publicodes-aom-bordeaux/      # Bordeaux transport tariffs
├── publicodes-aom-rennes/        # Rennes transport tariffs
├── publicodes-entreprise-innovation/  # Innovation aids (CIR, CII, JEI)
└── publicodes-apl/               # Housing aids (incomplete)
```
...
├── src/                      # Source .publicodes files
│   ├── variables.publicodes
│   ├── helpers.publicodes
│   ├── resultat.publicodes
│   └── dispositifs/          # Individual aid types
├── situations/               # Test scenarios
└── publicodes-build/         # Compiled output (generated)
    ├── index.js
    ├── index.d.ts
    └── <name>.model.json
```
...
- **`name`**: NPM package identifier (e.g., `@betagouv/publicodes-aom-bordeaux`)
- **`main`**: Entry point (`publicodes-build/index.js`)
- **`types`**: TypeScript definitions (`publicodes-build/index.d.ts`)
- **`files`**: Published files (`["publicodes-build"]`)
- **`scripts.compile`**: Build command (`publicodes compile`)
- **`peerDependencies`**: `publicodes` engine version
- **`devDependencies`**: `@publicodes/tools` for compilation

### Build System

The build script (`scripts/build_publicodes.js`) handles compilation:

**Process:**
1. Reads package list from script
2. For each package:
   - Runs `pnpm install` in package directory
   - Runs `pnpm run compile` (executes `publicodes compile`)
   - Generates `publicodes-build/` folder with compiled rules
3. Reports success/failure for each package

**Command:**
```bash
pnpm build:publicodes  # Builds all packages
```

**Current packages built:**
...
The `getPublicodesRules()` utility maps simulateur slugs to package imports:

**Location:** `inertia/utils/get_publicodes_rules.ts`

**Mapping:**
```typescript
'entreprise-innovation' → import('~packages/publicodes-entreprise-innovation')
'aom-rennes' → import('~packages/publicodes-aom-rennes')
'aom-bordeaux' → import('~packages/publicodes-aom-bordeaux')
```

**Alias resolution:**
- `~packages/` → Vite alias pointing to `./packages/` folder
- Configured in `vite.config.ts` and `inertia/tsconfig.json`
- Imports from local builds (not npm packages currently)

### Local vs NPM Packages

**Current approach (Local):**
- Packages built locally with `pnpm build:publicodes`
- Imported via `~packages/` alias pointing to local folders
- No npm installation needed
- Development-friendly for package changes

**Future approach (NPM):**
- Packages published to npm registry
- Installed as dependencies in main `package.json`
- Imported via package name (e.g., `@betagouv/publicodes-aom-bordeaux`)
- Production-ready, faster builds
- Requires switching import paths in `getPublicodesRules()`
...
- **Input**: `.publicodes` files in `packages/publicodes-<name>/src/`
- **Output**: `publicodes-build/` directory with:
  - `index.js`: Compiled rules as JavaScript module
  - `index.d.ts`: TypeScript type definitions
  - `<name>.model.json`: JSON representation of rules
- **Process**: `pnpm run compile` in each package directory
- **Installed as**: `devDependency` in each package (`@publicodes/tools: ^1.7.2`)
...
- Rules defined in `packages/publicodes-<slug>/` YAML files
...
```
Publicodes Package (YAML files in packages/)
  ├─→ Compiled Rules (JS module in publicodes-build/)
  ├─→ Form Schema (extracted from rules)
  └─→ Calculation Logic (engine)

Simulateur (usesPublicodesForms = true)
  ├─→ Publicodes Package (by slug via getPublicodesRules())
  └─→ FormSubmission (stores results)
```

### Package to Simulateur Mapping
Each Publicodes simulateur corresponds to one package:
- `aom-bordeaux` → `/simulateurs/aom-bordeaux` → `packages/publicodes-aom-bordeaux`
- `aom-rennes` → `/simulateurs/aom-rennes` → `packages/publicodes-aom-rennes`
- `entreprise-innovation` → `/simulateurs/entreprise-innovation` → `packages/publicodes-entreprise-innovation`

Mapping defined in `getPublicodesRules()` utility in `inertia/utils/get_publicodes_rules.ts`.
...
**Developer workflow**:
1. Author rules in `packages/publicodes-<slug>/src/*.publicodes`
2. Run `pnpm build:publicodes`
3. Script iterates through all Publicodes packages
4. Each package compiles via `pnpm run compile`
5. Output generated in `packages/publicodes-<slug>/publicodes-build/`

**Build script** (`scripts/build_publicodes.js`):
- Installs dependencies for each package (`pnpm install` in package folder)
- Runs `@publicodes/tools` compiler (`pnpm run compile`)
- Generates TypeScript typings (`index.d.ts`)
- Creates optimized JSON model (`<name>.model.json`)

**Workspace integration**:
- Packages located in `packages/` folder
- Main app imports via alias: `~packages/publicodes-<name>`
- Vite bundles compiled rules into main app
- Vite alias configured in `vite.config.ts`: `'~packages/': .../packages/`
- TypeScript paths configured in `inertia/tsconfig.json`: `"~packages/*": ["../packages/*"]`
...
**Dynamic import** (`inertia/utils/get_publicodes_rules.ts`):
- Matches slug to package name via switch statement
- Performs dynamic import: `import('~packages/publicodes-aom-bordeaux')`
...
### Frontend Utilities
- `inertia/utils/get_publicodes_rules.ts`: Dynamic rule loader, maps simulateur slugs to package imports
...
### Build Scripts
- `scripts/build_publicodes.js`: Compiles all Publicodes packages
- `scripts/build_logger.js`: Logging utility for build process

### Tests
- `packages/publicodes-<name>/situations/`: Test case definitions
- `tests/e2e/02_simulation.spec.ts`: End-to-end Publicodes flow

## Adding a New Publicodes Package
...
```bash
mkdir -p packages/publicodes-<name>/src/dispositifs
```
...
```json
{
  "name": "@betagouv/publicodes-<name>",
  "type": "module",
  "version": "1.0.0",
  "description": "Modèle publicodes de <description>",
...
### 3. Create Publicodes Rules

Create `.publicodes` files in `src/`:
- `variables.publicodes` - Input variables
- `helpers.publicodes` - Helper calculations
- `resultat.publicodes` - Result formatting
- `dispositifs/*.publicodes` - Individual aid rules

### 4. Add to Build Script

Edit `scripts/build_publicodes.js`:
...
const publicodesPackages = [
  ...
  'publicodes-aom-rennes',
  'publicodes-entreprise-innovation',
  'publicodes-<name>',  // Add new package
]
```

### 5. Add to Import Map

Edit `inertia/utils/get_publicodes_rules.ts`:

```typescript
switch (simulateurSlug) {
  case '<slug>':
    return import('~packages/publicodes-<name>').then(module => module.default)
  // ... other cases
}
```
...
```bash
pnpm build:publicodes        # Compile the package
...
- **devDependencies**: `@publicodes/tools` must be `^1.7.2` (not dependency)
- **peerDependencies**: `publicodes` engine version
- **files**: Only publish `publicodes-build/` folder
- **scripts**: `compile` command for building
- **structure**: Same folder organization for maintainability
