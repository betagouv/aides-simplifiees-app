          ...
          <div>
            <div class="fr-grid-row fr-grid-row--right fr-mb-2w">
              <DsfrButton
                ...
                :onclick="() => {router.visit(`/admin/aides/create`)}"
              />
            </div>

            <h2 class="fr-mb-3w">Liste des aides</h2>

            <div v-if="aides.length === 0" class="fr-alert fr-alert--info">
              <p>Aucune aide trouvée</p>
            </div>

            <div v-else class="aide-list">
              <div v-for="aide in aides" :key="aide.id" class="fr-grid-row fr-grid-row--gutters fr-mb-2w aide-item">
                <div class="fr-col-12 fr-p-2w">
                  <div class="fr-grid-row fr-grid-row--middle">
                    <div class="fr-col-12 fr-col-md-8">
                      <h3 class="fr-mb-0">{{ aide.title }}</h3>
                      <p class="fr-mb-0 badges-row">
                        <span class="fr-badge fr-badge--sm fr-badge--blue-cumulus fr-mr-1w">#{{ aide.id }}</span>
                        <span v-if="aide.type" class="fr-badge fr-badge--green-emeraude fr-mr-1w">{{ aide.type }}</span>
                        <span v-if="aide.instructeur" class="fr-badge fr-badge--brown-cafe-creme fr-mr-1w">{{ aide.instructeur }}</span>
                      </p>
                      <p class="fr-mb-0">
                    ...
                    <div class="fr-col-12 fr-col-md-4 fr-grid-row fr-grid-row--right fr-mt-2w fr-mt-md-0">
                      <div class="fr-btns-group fr-btns-group--inline">
                        <DsfrButton
                          ...
                          size="sm"
                          :onclick="() => {router.visit(`/admin/aides/${aide.id}/edit`)}"
                        />
                        <DsfrButton
                          label="Voir"
                          ...
                          size="sm"
                          secondary
                          :onclick="() => {router.visit(`/aides/${aide.slug}`)}"
                        />
                      </div>
                    </div>
                  </div>
